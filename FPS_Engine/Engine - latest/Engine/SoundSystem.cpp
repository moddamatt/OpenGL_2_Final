...
Sound::Sound(char *filename)
{
	if (!alutInit(0, NULL))
	{
		...
		MessageBox(NULL, "cant init openal", "error", MB_OK);
		exit(EXIT_FAILURE);
	}
	...
	if (buffer == AL_NONE)
	{
		...
		fprintf(stderr, "Error loading file: '%s'\n",
			alutGetErrorString(error));
		MessageBox(NULL, "cant load sound", "error", MB_OK);
		alutExit();
		exit(EXIT_FAILURE);
	}
...
//{
//	// Clear the sound buffer and audio path pointers.
//	m_soundBuffer = NULL;
//	m_audioPath = NULL;
//
