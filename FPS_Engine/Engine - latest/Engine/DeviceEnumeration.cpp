		...
		case WM_INITDIALOG:
		{
			...
			else
			{
				...
				if( m_windowed == false )
				{
					...
					EnableWindow(GetDlgItem(dialog, IDC_COLOR_DEPTH), true);
					...
					ComboBox_ResetContent( GetDlgItem( dialog, IDC_COLOR_DEPTH ) );
					m_displayModes->Iterate( true );
					while( m_displayModes->Iterate() )
						if( !ComboBoxContainsText( dialog, IDC_COLOR_DEPTH, m_displayModes->GetCurrent()->bpp ) )
							ComboBoxAdd( dialog, IDC_COLOR_DEPTH, (void*)m_displayModes->GetCurrent()->mode.Format, m_displayModes->GetCurrent()->bpp );
					ComboBoxSelect( dialog, IDC_COLOR_DEPTH, *m_settingsScript->GetNumberData( "bpp" ) );
					...
					while( m_displayModes->Iterate() )
					{
						if( m_displayModes->GetCurrent()->mode.Format == (D3DFORMAT)PtrToUlong( ComboBoxSelected( dialog, IDC_COLOR_DEPTH ) ) )
						{
		...
		case WM_COMMAND:
		{
			switch( LOWORD(wparam) )
			{
				case IDOK:
				{
					...
					m_selectedDisplayMode.Format = (D3DFORMAT)PtrToUlong( ComboBoxSelected( dialog, IDC_COLOR_DEPTH ) );
					...
					long bpp = ComboBox_GetCurSel( GetDlgItem( dialog, IDC_COLOR_DEPTH ) );
				...
				case IDC_FULLSCREEN:
				{
					...
					if( IsDlgButtonChecked( dialog, IDC_WINDOWED ) )
					{
						...
						EnableWindow( GetDlgItem( dialog, IDC_COLOR_DEPTH ), false );
					...
					else
					{
						...
						ComboBoxSelect( dialog, IDC_COLOR_DEPTH, 0 );
					}
