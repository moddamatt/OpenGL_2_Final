...
AnimatedObject(script->GetStringData("mesh_name"), script->GetStringData("mesh_path"), type)
{
	...
	//m_leftStepAudioPath = new Sound;
	//m_rightStepAudioPath = new Sound;
...
void PlayerObject::Hurt(float damage, PlayerObject *attacker)
{
	...
	PlayerHealthMsg phm;
	phm.msgid = MSGID_PLAYER_HEALTH;
	phm.dpnid = m_dpnid;
	phm.health = m_health;
	g_engine->GetNetwork()->Send(&phm, sizeof(PlayerHealthMsg), DPNID_ALL_PLAYERS_GROUP, DPNSEND_NOLOOPBACK);
...
void PlayerObject::ChangeWeapon(char change, char weapon)
{
	...
	NetworkMessage pwcm;
	pwcm.msgid = MSGID_PLAYER_WEAPON_CHANGING;
	pwcm.dpnid = g_engine->GetNetwork()->GetLocalID();
