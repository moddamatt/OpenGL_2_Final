...
public:
	...
	void MouseLook(float elapsed, float x, float y, bool reset = false);
...
private:
	...
	DPNID m_dpnid; // DirectPlay ID of the player.
	...
	char m_oldWeapon;
	D3DXVECTOR3 m_viewWeaponOffset;
	float m_changingWeapon;
	bool m_weaponChanging;
	...
	//Sound *m_leftStepAudioPath; // Audio path for playing the left foot step sound.
	//Sound *m_rightStepAudioPath; // Audio path for playing the right foot step sound.
};
